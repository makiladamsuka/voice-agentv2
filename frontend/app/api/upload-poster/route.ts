...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (!response.ok) {
        console.error('Failed to trigger index in backend:', response.statusText);
      } else {
    ...
    return NextResponse.json({ success: true, message: 'Poster uploaded successfully.', fileName });
  } catch (error: any) {
