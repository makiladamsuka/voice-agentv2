...
import { OrbitControls, Line, RoundedBox, Grid, Text, Sparkles, Environment } from '@react-three/drei';
...
const GlowingPath = ({ points }: { points: [number, number, number][] }) => {
  ...
  return (
    <group>
      <Line ref={lineRef} points={points} color="#ef4444" lineWidth={8} dashed dashSize={0.5} gapSize={0.3} />
      {/* Moving energy orb */}
      <mesh ref={orbRef}>
...
export default function NavigationMap({ path, nodes, buildings, destination, onClose, inline = false }: NavigationMapProps) {
  ...
  return (
    <div 
      className={inline ? "w-full h-full relative" : `fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center transition-all duration-500 ${
    ...
    >
      ...
      <div className="w-full h-full" onClick={(e) => e.stopPropagation()}>
        <Canvas shadows orthographic camera={{ position: [20, 20, 20], zoom: 35 }}>
          ...
          <Sparkles count={200} scale={30} size={1.5} speed={0.4} opacity={0.2} color="#818cf8" />
          ...
          {nodes.filter((n: any) => n.type !== 'waypoint' && n.floor === (destNode?.floor || 'floor_1')).map((node: any) => {
            ...
            if (lbl.includes('lab') || lbl.includes('laboratory')) roomColor = '#c084fc'; // Purple for Labs
            else if (lbl.includes('lec') || lbl.includes('hall') || lbl.includes('auditorium')) roomColor = '#fb923c'; // Orange for Lecture Halls
            else if (lbl.includes('dept') || lbl.includes('department') || lbl.includes('office') || lbl.includes('unit') || lbl.includes('desk')) roomColor = '#38bdf8'; // Light Blue for Departments
            else if (lbl.includes('washroom') || lbl.includes('restroom')) roomColor = '#94a3b8'; // Slate for Washrooms

            return (
              <group key={node.id} position={[node.world[0], node.world[1], node.world[2]]}>
                <RoundedBox args={size} radius={0.1} smoothness={4} castShadow>
                  {isDestination ? (
                    <meshStandardMaterial color="#22c55e" emissive="#22c55e" emissiveIntensity={0.5} transparent opacity={0.9} />
                  ) : (
                ...
                <Text position={[0, size[1] / 2 + 0.4, 0]} fontSize={0.35} color={isDestination ? "#22c55e" : "#cbd5e1"} anchorX="center" anchorY="middle" outlineWidth={0.02} outlineColor="#000000">
                  {node.label}
                </Text>
              </group>
            );
          ...
          <OrbitControls enableZoom={true} enablePan={true} maxPolarAngle={Math.PI / 2 - 0.1} autoRotate autoRotateSpeed={1.5} />
        </Canvas>
      </div>
